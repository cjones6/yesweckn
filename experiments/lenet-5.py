...
"""
Train the LeNet-5 CKN on MNIST
"""
...
parser.add_argument('--data_path', default='../data/MNIST', type=str,
                    help="Location of the MNIST dataset. If it doesn't exist it will be automatically downloaded "
                         "there.")
parser.add_argument('--save_path', default='../results/lenet-5/temp', type=str,
...
params = parse_config.load_config('../cfg/lenet-5.cfg')
...
# For LeNet-5, layers 1 and 3 use the linear kernel and so their filters aren't trained.
train = train_supervised.TrainSupervised(train_loader, valid_loader, train_valid_loader, test_loader, model,
                                         nclasses=10, loss_name=args.loss.lower(), train_w_layers=[0, 2, 4, 5],
                                         update_lr_freq=args.update_lr_every, tau=args.hessian_reg,
                                         maxiter_outer=args.max_iter, save_path=args.save_path,
...
plt.ylabel('Accuracy')
plt.title('Accuracy of LeNet-5 on MNIST with ' + str(args.num_filters) + ' filters per layer')
plt.show()
