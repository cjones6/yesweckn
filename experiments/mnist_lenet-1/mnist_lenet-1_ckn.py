"""
Train a LeNet-1 CKN on the MNIST dataset
"""
...
parser = argparse.ArgumentParser(description='LeNet-1 CKN training on the MNIST dataset')
parser.add_argument('--batch_size', default=4096, type=int,
                    help='Batch size for training')
parser.add_argument('--data_path', default='../../data/mnist', type=str,
                    help='Location of the MNIST dataset')
parser.add_argument('--diag_hessian', default=0, type=int,
                    help='Whether to use a diagonal approximation of the Hessian')
...
parser.add_argument('--save_path', default='../../results/mnist_lenet-1_ckn/', type=str,
...
save_file = os.path.join(save_dir, str(bw) + '_' + str(args.diag_hessian) + '_' + str(args.hessian_reg)
                         + '_' + str(args.num_filters) + '_' + str(args.seed) + '_' + str(args.step_size)
                         + '_' + str(time.time()))
...
params = parse_config.load_config('../../cfg/lenet-1_ckn.cfg')
...
params = opt_structures.Params(num_classes=num_classes, ckn=True, train_w_layers=[0, 2], lambda_filters=0,
                               ...
                               tau=args.hessian_reg, diag_hessian=args.diag_hessian, num_iters=args.num_iters,
