"""
Train a LeNet-1 ConvNet on the MNIST dataset
"""
...
from src.data_loaders import mnist
from src.model.convnet import lenet
from src.opt import opt_structures, train_network

parser = argparse.ArgumentParser(description='LeNet-1 ConvNet training on the MNIST dataset')
...
parser.add_argument('--hessian_reg', default=-2, type=int,
                    help='log2(Regularization of the Hessian for the ULR-SGO method)')
parser.add_argument('--incomplete_scheme', default=0, type=int,
                    help='Whether to use the incomplete connection scheme from the original LeNet-1')
parser.add_argument('--lambda_filters', default=-8, type=int,
                    help='log2(L2 penalty for the filters)')
parser.add_argument('--num_filters', default=None, type=int,
                    help='Number of filters per layer')
parser.add_argument('--num_iters', default=10000, type=int,
                    help='Number of total iterations to perform')
parser.add_argument('--save_path', default='../../results/mnist_lenet-1_convnet/', type=str,
...
save_file = os.path.join(save_dir, str(args.hessian_reg) + '_' + str(args.incomplete_scheme)
                         + '_' + str(args.lambda_filters) + '_' + str(args.num_filters) + '_' + str(args.seed)
...
model = lenet.LeNet1(nfilters=args.num_filters, locally_connected_layer=args.incomplete_scheme, bias=True)
...
if args.step_size_method == 'fixed':
    ...
    def lambda_filters_schedule(x):
        if x < 2000:
            ...
            return 2 ** -8
    update_step_size_method = 'schedule'
elif args.step_size_method == 'best-convnet-incomplete':
    def step_size_schedule(x):
        if x < 2000:
            return 2 ** -4
        elif 2000 <= x < 4000:
