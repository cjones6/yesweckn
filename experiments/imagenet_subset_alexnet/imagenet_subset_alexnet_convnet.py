"""
Train an AlexNet ConvNet on our subset of ImageNet dataset
"""
...
from src.model.convnet import alexnet
from src.opt import opt_structures, train_network

# Parameters for the model, data, and training
parser = argparse.ArgumentParser(description='AlexNet ConvNet training on our subset of ImageNet')
...
parser.add_argument('--save_path', default='../../results/imagenet_subset_alexnet_convnet/', type=str,
...
model = alexnet.AlexNet(nfilters=args.num_filters, bias=True).to(defaults.device)
if torch.cuda.device_count() > 1:
    model = nn.DataParallel(model).to(defaults.device)
    model.module.apply(alexnet.init_weights)
else:
    model.to(defaults.device).apply(alexnet.init_weights)
